...
public class AuthService {
    ...
    public Uni<String> authenticate(AuthRequest authRequest) {
        return userService.findByName(authRequest.name()).onItem().transform(user -> {
            if (user == null || !UserService.matchs(user, authRequest.password())) {
                throw new AuthenticationFailedException("Invalid confidentails");
            }
            return Jwt.issuer(issuer).upn(user.name).groups(new HashSet<>(user.roles)).expiresIn(Duration.ofHours(1L)).sign();
        });
    }
}