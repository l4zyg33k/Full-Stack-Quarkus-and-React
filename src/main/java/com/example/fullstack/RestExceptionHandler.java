...
public class RestExceptionHandler implements ExceptionMapper<HibernateException> {
  ...
  public Response toResponse(HibernateException e) {
    ...
    if (hasExceptionInChain(e, StaleObjectStateException.class)
        || hasPostgresErrorCode(e, PG_UNIQUE_VIOLATION_ERROR)) {
  ...
  private boolean hasPostgresErrorCode(Throwable throwable, String sqlState) {
    return getExceptionInChain(throwable, java.sql.SQLException.class)
        .filter(ex -> Objects.equals(ex.getSQLState(), sqlState))
        .isPresent();
  }
}
